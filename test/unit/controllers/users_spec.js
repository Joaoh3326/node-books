...
describe("Controller: User", () => {
  ...
  describe("create() user", () => {
    it("should call send with a new user", async () => {
      const requestWithBody = Object.assign(
        {},
        { body: defaultUser[0], defaultRequest }
      );
      const response = {
        send: sinon.spy(),
        status: sinon.stub()
  ...
  describe('update() user', () => {
    it('should reutrn with 200 when the user has been updated', async () => {
      const fakeId = 'a-fake-id';
      const updatedUser = {
        _id: fakeId,
        name: 'Updated User',
  ...
  describe('authenticate', () => {
    it('should authenticate an user', async () => {
      ...
      const userWithEncryptedPassord = {
        ...user,
        password: bcrypt.hashSync(user.password, 10)
      };
      const jwtToken = jwt.sign(
        userWithEncryptedPassord,
        config.get('auth.key'),
        {
      ...
      class FakeAuthService {
        authenticate() {
          return Promise.resolve(userWithEncryptedPassord);
        }
